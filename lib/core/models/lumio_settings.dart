...
class LumioSettings {
  ...
  factory LumioSettings.fromJson(Map<String, Object?> json) {
    return LumioSettings(
      ...
      musicViewMode: _musicViewModeFromName(
  ...
  LumioSettings withCrossfadeEnabled(bool enabled) {
    return copyWith(
      crossfadeSeconds: enabled
          ? crossfadeSeconds > 0
              ? crossfadeSeconds
              : 3
          : 0,
...
MusicViewMode _musicViewModeFromName(String? name) {
  return MusicViewMode.values.firstWhere(
    (value) => value.name == name,
    orElse: () => MusicViewMode.list,
  );
}

ThemeAccent _themeAccentFromName(String? name) {

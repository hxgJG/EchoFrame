# EchoFrame Phase 8i Fullscreen Video Controls Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add synced playback controls to the fullscreen video route.

**Architecture:** Keep fullscreen playback state driven by `EchoAppState`. The fullscreen route already listens through `AnimatedBuilder`; add play/pause, previous/next, current time, duration, and draggable seek controls that call existing AppState methods.
...
**Interfaces:**
- Consumes: `EchoAppState.togglePlaying`
- Consumes: `EchoAppState.previous`
- Consumes: `EchoAppState.next`

# Lumio Phase 8j Subtitle Style Settings Implementation Plan
...
**Architecture:** Persist subtitle style options in `LumioSettings` and expose AppState setters. The video stage and fullscreen video route read the current settings when rendering subtitle overlays. This keeps SRT parsing unchanged and avoids ASS style parsing or third-party dependencies.
...
- Produces: `LumioAppState.setSubtitleFontSize(double size)`
- Produces: `LumioAppState.setSubtitleTextColor(SubtitleTextColor color)`
- Produces: `LumioAppState.setSubtitlePosition(SubtitlePosition position)`
...
**Interfaces:**
- Consumes: `LumioAppState.settings.subtitleFontSize`
- Consumes: `LumioAppState.settings.subtitleTextColor`
- Consumes: `LumioAppState.settings.subtitlePosition`

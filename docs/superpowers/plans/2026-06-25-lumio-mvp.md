# Lumio MVP Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the first runnable Flutter MVP for Lumio with offline-first music/video navigation, local demo media state, settings, and a documented path to real Android scanning/playback.
...
- Create: `docs/superpowers/plans/2026-06-25-lumio-mvp.md`

**Interfaces:**
- Produces: a Flutter app named `lumio` with entrypoint `main()` and a technical plan.
- Consumes: `docs/Lumio_PRD_v0.5.md`.

- [x] **Step 1: Generate Flutter scaffold**

```bash
flutter create --project-name lumio --org com.hxg.lumio --platforms=android,ios,ohos --no-pub .
```
...
- Create: `lib/core/models/lumio_settings.dart`
- Create: `lib/app/seed_data.dart`
- Create: `lib/app/app_state.dart`

**Interfaces:**
- Produces: `LumioAppState extends ChangeNotifier`
...
Implement immutable `Playlist` and `LumioSettings` with `copyWith`.
...
**Interfaces:**
- Consumes: `LumioAppState`
- Produces: `LumioApp`, `LumioShell`, `LumioTheme.light`, `LumioTheme.dark`
...
Wire `MaterialApp`, `AnimatedBuilder` for `LumioAppState`, bottom navigation, and persistent Mini Player above navigation.
...
**Interfaces:**
- Consumes: `LumioAppState`, `MediaItem`, `Playlist`
...
- Produces: Android MethodChannel `lumio/media_library` method `scan`.
- Produces: Android MethodChannel `lumio/media_library` method `restoreLastScan`.
...
Add a settings row that triggers `LumioAppState.scanMediaLibrary()`, shows scanning state, and replaces seed data with scan results.
...
- Modify: `docs/superpowers/plans/2026-06-25-lumio-mvp.md`
